...
-- Milestone 150: Paladins' Consecration also detonates instantly for its full
...
-- (spell_dbc/talent_dbc rows + the client patch-5 MPQs) comes from
...
-- Consecration burst (milestone 150, Paladin): every Consecration cast also
...
-- — so the doubling holds with gear). Generated by
-- Tools/gen_consecration_burst.py. Totals below = per-tick DBC damage x 8.
...
local LEARNED_SPELL_SPECIALS = {
    ...
    -- client patch-5 MPQs; the whole mechanic is spell data, zero Lua)

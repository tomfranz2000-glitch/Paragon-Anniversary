...
- 1900037 is generated by `Tools/paragon_client_patch.py` **`SERVER_SPELLS`**
  ...
  entry, the aura is invisible like the 1900003–05 family). Applied via
...
## 2. Custom server-side spells (`acore_world.spell_dbc`)

These rows exist **only in the live database**. A world-DB re-import or reset
drops them, which silently disables the three reward-track bonuses that depend
on them. The client has no matching Spell.dbc entries, so all three are
invisible in the buff bar by design — the paragon window documents them.
...
| ID | Name | Effect |
|----|------|--------|
| 1900003 | Paragon Swiftness | `EffectAura_1 = 31` (MOD_INCREASE_SPEED), +50% run speed |
| 1900004 | Paragon Aquatic Grace | `EffectAura_1 = 58` (MOD_INCREASE_SWIM_SPEED), +100% swim speed |
| 1900005 | Paragon Quick Mount | `EffectAura_1 = 4` (DUMMY) — pure marker, read by the patch in §1 |
...
1900014 (generated by `Tools/gen_consecration_burst.py`, clone of Holy Nova's
...
logic (which Consecration ranks trigger it, damage totals, the paragon-150
...
and both patch-5 MPQs. Notes learned the hard way: `spell_dbc` mask columns
...
3. If the spell feeds another system (Consecration → the milestone-150
...
registered in `CONSECRATION_BURST.totals` (1176) so the milestone-150 burst
fires for it — that coupling must be maintained for any future Consecration
rank.

Common column choices: `DispelType = 0` (immune to purge/spellsteal),
`DurationIndex = 21` (permanent), `ImplicitTargetA_1 = 1` (self),
`RangeIndex = 1`, `EquippedItemClass = -1`, `Effect_1 = 6` (APPLY_AURA),
`EffectDieSides_1 = 1`, and `EffectBasePoints_1 = value - 1` (the core adds 1).

```sql
DELETE FROM spell_dbc WHERE ID IN (1900003, 1900004, 1900005);
INSERT INTO spell_dbc
    (ID, DispelType, ProcChance, DurationIndex, RangeIndex, EquippedItemClass,
     Effect_1, EffectDieSides_1, EffectBasePoints_1, ImplicitTargetA_1, EffectAura_1,
     Name_Lang_enUS, SchoolMask)
VALUES
    (1900003, 0, 101, 21, 1, -1, 6, 1, 49, 1, 31, 'Paragon Swiftness', 1),
    (1900004, 0, 101, 21, 1, -1, 6, 1, 99, 1, 58, 'Paragon Aquatic Grace', 1),
    (1900005, 0, 101, 21, 1, -1, 6, 1,  0, 1,  4, 'Paragon Quick Mount', 1);
```

To have these survive a database rebuild automatically, move the statement into
`data/sql/custom/` (gitignored) instead of applying it by hand.
...
`acore_world.spell_dbc` (server) and the client patch-5 MPQs. Optional
